...
# dependencies = ["magika", "anthropic", "openai"]
# ///
...
Usage:
    # Audit eval JSONL with Anthropic Haiku
    source ~/.a.sh
    uv run training/audit_labels_vote.py --input training/data/eval/clear.jsonl \
        --output training/output/label_votes_clear.jsonl --backend anthropic

    # Audit training CSV with OpenRouter gpt-5.4-mini (fast, concurrent)
    source ~/.a.sh
    uv run training/audit_labels_vote.py \
        --input training/data/curated/train/golden_raw.csv \
        --output training/output/label_votes_train.jsonl \
        --backend openrouter --model openai/gpt-5.4-mini --concurrency 10
"""
