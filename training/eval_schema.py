...
def extract_provenance_fields(sample: dict) -> dict:
    """Return the provenance fields present in *sample*."""
    return {field: sample.get(field) for field in PROVENANCE_FIELDS if field in sample}


def build_prediction_record(sample: dict, predicted_category: str) -> dict:
    """Project an eval sample into a prediction record."""
    record = {
    ...
    record.update(extract_provenance_fields(sample))
    return record
