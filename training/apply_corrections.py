...
Usage:
    ...
    uv run training/apply_corrections.py \
        --votes training/output/label_votes_clear.jsonl \
        --input training/data/eval/clear.jsonl \
        --output training/data/eval/clear_v2.jsonl

    # Apply to training CSV
    uv run training/apply_corrections.py \
        --votes training/output/label_votes_train.jsonl \
        --input training/data/curated/train/golden_raw.csv \
        --output training/data/curated/train/golden_raw_v2.csv

    # Remap ghost sub-types only (no vote file)
    uv run training/apply_corrections.py \
        --input training/data/curated/train/golden_raw.csv \
        --output training/data/curated/train/golden_raw_v2.csv \
        --remap-subtypes-only
