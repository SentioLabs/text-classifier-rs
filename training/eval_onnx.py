...
# dependencies = ["onnxruntime", "numpy", "tqdm"]
# ///
...
def run_evaluation(
    ...
    *session* is an ``ort.InferenceSession`` (or mock with compatible API).
    Returns a metrics dict produced by :func:`compute_metrics`.
    """
    inv_map = invert_category_map(config["category_map"])
    categories = sorted(config["category_map"].keys(), key=lambda c: config["category_map"][c])
...
def main(argv: list[str] | None = None) -> None:
    ...
    for eval_path in args.eval:
        ...
        metrics = run_evaluation(session, config, samples)
