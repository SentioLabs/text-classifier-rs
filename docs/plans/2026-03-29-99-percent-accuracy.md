...
## Shared Contracts (Parallel Readiness)

Phases 2 and 3 run in parallel. These contracts are the shared interfaces between them. All exist today except the eval JSONL schema — that is the only new contract introduced.

### 1. Feature Vector Ordering (existing — do not modify)

The 18-feature array in `tier2.rs:feature_vector_to_array()` defines the canonical ordering. Training (`train.py`) and inference (`tier2.rs`) MUST use identical ordering. Neither Phase 2 nor Phase 3 modifies this.

### 2. model_config.json Schema (existing — do not modify)

```json
{
  "feature_names": ["line_length_cv", "char_entropy", ...],
  "feature_mean": [float, ...],
  "feature_std": [float, ...],
  "category_map": {"prose": 0, "code": 1, "structured": 2, "artifact": 3, "skip": 4},
  "sub_type_map": {"boilerplate": 0, "csv": 1, ...}
}
```

Training exports this; Rust inference reads it via `include_str!`.

### 3. ONNX Model I/O (existing — do not modify)

- Input: `"features"` tensor, shape `[batch, 18]`, dtype `f32`
- Output 0: `category_logits`, shape `[batch, 5]`
- Output 1: `sub_type_logits`, shape `[batch, N_subtypes]`

### 4. TextCategory Enum <-> category_map Keys (existing — keep in sync)

- Rust: `TextCategory::Prose`, `Code`, `Structured`, `Artifact`, `Skip`
- Python: `"prose"`, `"code"`, `"structured"`, `"artifact"`, `"skip"`

### 5. Eval JSONL Schema (NEW — introduced in Phase 1)

```json
{"text": "...", "expected_category": "prose", "boundary_pair": null}
{"text": "...", "expected_category": "code", "boundary_pair": "code_structured"}
```

- `text`: the raw text to classify
- `expected_category`: ground-truth label (one of: prose, code, structured, artifact)
- `boundary_pair`: null for clear samples, `"<cat1>_<cat2>"` for boundary samples

### T0 Foundation Task

Minimal: define the eval JSONL schema (contract #5) and add a shared validation function in `generate.py` that verifies samples conform to it. All other contracts are already stable.
